...
class PopularTv extends StatefulWidget {
  PopularTv({Key? key}) : super(key: key);
...
class _PopularTvState extends State<PopularTv> {
  ...
  void didChangeDependencies() {
    if (_isInit) {
      ...
      Provider.of<TvShowsProvider>(context).getPopular().then((_) {
        setState(() {
          _isLoading = false;
        });
      });
  ...
  Widget build(BuildContext context) {
    ...
    _pageController.addListener(() {
      if (_pageController.offset >= _pageController.position.maxScrollExtent &&
        ...
        // reached bottom
        if (newPage < 3) {
          temp.addMorePopularTv(nextPage: newPage);
          newPage += 1;
        } else {
          temp.setNoMore = true;
        }
                  ...
                  child: InkWell(
                      onTap: () => Navigator.of(context).push(MaterialPageRoute(
                          builder: (_) => TvShowDetailsScreen(
                              tvId: tvProvider.popularLS[index].id))),
                                ...
                                child: ClipRRect(
                                    ...
                                    child: BlurHash(
                                      ...
                                      image: 'https://image.tmdb.org/t/p/w500' +
                                          tvProvider
                                              .popularLS[index].posterPath,
                                    )))),
                        Padding(
                          padding:
                              const EdgeInsets.only(top: kDefaultPadding / 2),
                          child: Text(tvProvider.popularLS[index].name,
                        ...
                        Padding(
                            ...
                            child: Row(
                                ...
                                children: [
                                  ...
                                  Text(
                                      tvProvider.popularLS[index].voteAverage
                                          .toString(),
                                      style:
